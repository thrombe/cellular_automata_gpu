...
fn get_index(x: i32, y: i32, dx: i32, dy: i32) -> u32 {
    let width = i32(stuff.render_width);
    var new_x = x;
    if (x >= width) {
        new_x = 0;
    } else if (x < 0) {
    ...
    let height = i32(stuff.render_height);
    var new_y = y;
    if (y >= height) {
        new_y = 0;
    } else if (y < 0) {
    ...
    return u32(((new_y/scale)+dy)*i32(stuff.render_width) + ((new_x/scale)+dx));
}
...
fn get_cell_stat_from_mouse(x: i32, y: i32, ori_state: u32) -> u32 {
    ...
    // if (((stuff.mouse_left == 1u) || (stuff.mouse_right == 1u)) && (get_index(i32(stuff.cursor_x), i32(stuff.cursor_y), 0, 0) == get_index(x, y, 0, 0))) {
    if (((stuff.mouse_left == 1u) || (stuff.mouse_right == 1u)) &&
        (length(v2f(stuff.cursor_x-f32(x), stuff.cursor_y-f32(y))) < stuff.scroll/1.0 + 5.0) &&
        (hash(u32(stuff.time*1000.0) + u32(x + y*i32(stuff.display_width)))%2u == 1u)) {
        state = 1u;
    }
...
fn main_fragment([[builtin(position)]] pos: vec4<f32>) -> [[location(0)]] vec4<f32> {
    ...
    var c = textureLoad(compute_texture, vec2<i32>(x, y));
    if (buf2.buf[index] == 1u) {
        c = v4f(1.0);
    } else if (c.x>0.03 && stuff.mouse_right == 0u) {
        c = c - 0.02;
    }
    textureStore(compute_texture, vec2<i32>(x, y), c);
